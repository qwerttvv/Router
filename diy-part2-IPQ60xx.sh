...
sed -i 's|^PKG_MIRROR_HASH.*|PKG_HASH:=skip|g' package/network/services/omcproxy/Makefile
sed -i 's|^PKG_SOURCE_VERSION.*|PKG_SOURCE_VERSION:=11e45f6cfe105fb3bacc5c38face79f2aa619984|g' package/network/services/omcproxy/Makefile

# Modify banner
sed -i 's|For more https://openwrt.org/docs/guide-user/additional-software/opkg-to-apk-cheatsheet|For more https://openwrt.org/docs/guide-user/additional-software/opkg-to-apk-cheatsheet\n\napk --no-cache --no-network add --allow-untrusted /tmp/tmp/*.apk|' package/base-files/files/etc/profile
