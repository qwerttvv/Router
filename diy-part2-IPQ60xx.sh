...
# 无WIFI配置调整Q6大小
find ./target/linux/qualcommax/dts/ -type f ! -iname '*nowifi*' -exec sed -i 's/ipq\(6018\|8074\).dtsi/ipq\1-nowifi.dtsi/g' {} +
...
sed -i 's|^PKG_MIRROR_HASH.*|PKG_HASH:=skip|g' package/network/services/omcproxy/Makefile
sed -i 's|^PKG_SOURCE_VERSION.*|PKG_SOURCE_VERSION:=cad82dc5cb65d7250dc998c5fd15c15187ba48ea|g' package/network/services/omcproxy/Makefile

# Modify banner
sed -i 's| %D %V, %C| %D %V, %C\n\n apk --no-cache --no-network add --allow-untrusted /tmp/tmp/*.apk\n|' package/base-files/files/etc/banner
